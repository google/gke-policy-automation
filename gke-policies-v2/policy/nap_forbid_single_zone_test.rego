...
test_cluster_not_enabled_nap if {
    nap_forbid_single_zone.valid with input as {"data": {"gke": {"name": "cluster-without-nap", "autoscaling": {"enable_node_autoprovisioning": false}}}}
}

test_cluster_enabled_nap_without_enabled_autoprovisioning_locations_not_enabled if {
    nap_forbid_single_zone.valid with input as {"data": {"gke": {"name": "cluster-with-nap", "autoscaling": {"enable_node_autoprovisioning": true}}}}
}

test_cluster_enabled_nap_with_enabled_autoprovisioning_locations_multiple if {
    nap_forbid_single_zone.valid with input as {"data": {"gke": {
...
test_cluster_enabled_nap_with_enabled_autoprovisioning_locations_single if {
    not nap_forbid_single_zone.valid with input as {"data": {"gke": {
        "name": "cluster-with-nap", 
        "autoscaling": {
            "enable_node_autoprovisioning": true, 
