...
violation contains msg if {
  ...
  not lower(pool.config.image_type) in {"cos", "cos_containerd"}
  not startswith(lower(pool.config.image_type), "windows")
  msg := sprintf("Node pool %q is not configured with COS", [pool.name])
} 
